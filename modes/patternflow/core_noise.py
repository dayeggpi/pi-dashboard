...
def perlin2d(x: float, y: float) -> float:
    ...
    return _lerpf(
        _lerpf(_grad2(_PERM[A],     x,       y),      _grad2(_PERM[B],     x - 1.0, y),      u),
        _lerpf(_grad2(_PERM[A + 1], x,       y - 1.0), _grad2(_PERM[B + 1], x - 1.0, y - 1.0), u),
        v,
    )


def fractal2d(x: float, y: float, octaves: int = 4, roughness: float = 0.5) -> float:
    total = amp = max_amp = 0.0; freq = 1.0; amp = 1.0
    for _ in range(octaves):
        total += perlin2d(x * freq, y * freq) * amp
        max_amp += amp; amp *= roughness; freq *= 2.0
